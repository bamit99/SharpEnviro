...
;Required .NET framework (4.8 is the version shipped with Windows 11)
...
Function setShellLeave
  ...
  IntCmp $SetupSetShell 0 DontSetShell
    ...
    # Ini File Mapping Change (HKEY_LOCAL_MACHINE needs administrator rights)
    ${If} ${RunningX64}
      SetRegView 64
    ${EndIf}
    ClearErrors
    ...
    IfErrors ShellMappingRetry ShellMappingDone
  ShellMappingRetry:
    # Retry once, the value can be locked while another process reads it
    ClearErrors
    WriteRegStr HKLM "Software\Microsoft\Windows NT\CurrentVersion\IniFileMapping\system.ini\boot" Shell "USR:Software\Microsoft\Windows NT\CurrentVersion\Winlogon"
    IfErrors ShellMappingFailed ShellMappingDone
  ShellMappingFailed:
    ${If} ${RunningX64}
      SetRegView 32
    ${EndIf}
    MessageBox MB_OK|MB_ICONEXCLAMATION "Setup was unable to write the shell redirect to the registry.$\n\
      Windows keeps the shell override in HKEY_LOCAL_MACHINE and writing it needs an elevated install (administrator rights).$\n\
      SharpEnviro has been installed, but Windows will keep starting Explorer as the shell.$\n\
      To change the shell, run this Setup again as administrator (right-click the Setup file and select 'Run as administrator')."
...
Function checkNETFramework
  !insertmacro MUI_HEADER_TEXT ".NET Framework 4.8 Setup" "SharpEnviro requires that the .NET Framework 4.8 is installed."
...
;Check for .NET framework (4.8 is the version shipped with Windows 11)
Function CheckFrameWork

  ;$0 is the return value ("1" = .NET Framework 4.8 or later is present), so it is
  ;deliberately not saved; only the scratch variables are pushed and popped.
  Push $1
  Push $2

  StrCpy $0 "0"
  StrCpy $2 "SOFTWARE\Microsoft\NET Framework Setup\NDP\v4\Full"

  ;.NET Framework 4.x stores its release number in the 32 bit registry view on x64 systems
  ${If} ${RunningX64}
    SetRegView 32
  ${EndIf}
  StrCpy $1 0
  ReadRegDWORD $1 HKLM "$2" "Release"
  IntCmp $1 ${MIN_FRA_RELEASE} fra_ok fra_other_view fra_ok

  ;Not found (or too old) in this view, the value may live in the 64 bit view instead
  fra_other_view:
  ${If} ${RunningX64}
    SetRegView 64
    StrCpy $1 0
    ReadRegDWORD $1 HKLM "$2" "Release"
    SetRegView 32
    IntCmp $1 ${MIN_FRA_RELEASE} fra_ok fra_end fra_ok
  ${EndIf}
  GoTo fra_end

  ;Version on machine is greater than or equal to what we need
  fra_ok:
  StrCpy $0 "1"
